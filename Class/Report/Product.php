<?php
    class Class_Report_Product extends Class_Report_Abstract 
    {
            function report()
        {
            $option="";
            ...
            $where=array();
            if($this->productId!='') $where[] = " p.entityId='".intval($this->productId)."'";
        ...
        function pie()
        {
           ...
           if(is_array($report)){
               foreach($report as $k =>$v)
                {
                    if($i==1) $userData .= '[\''.$v['productName'].'\','.$v['total'].']';
                    else  $userData .=  ',[\''.$v['productName'].'\','.$v['total'].']';
                    $i++;
                }
           }
           $userData.='] ';
